const w3color = (color: any, elmnt?: any) => {
  if (typeof color == "object") {
    return color;
  }
  w3color.prototype.attachValues(toColorObject(color));
  if (elmnt) {
    elmnt.style.backgroundColor = w3color.prototype.toRgbString();
  }
}
w3color.prototype = {
  ...
  isDark: function (n: any) {
    var m = n || 128;
    return (this.red * 299 + this.green * 587 + this.blue * 114) / 1000 < m;
  },
  saturate: function (n: any) {
    var x, rgb, color;
    x = n / 100 || 0.1;
    this.sat += x;
    if (this.sat > 1) {
  ...
  attachValues: function (color: any) {
    this.red = color.red;
    ...
    if(colornames != undefined){
      for (i = 0; i < colornames.length; i++) {
        if (c.toLowerCase() == colornames[i].toLowerCase()) {
          let colorhexs = getColorArr("hexs");
...
function colorObject(rgb: any, a: any, h: any, s: any) {
...
function ncolToRgb(ncol: any, white: any, black: any) {
...
function rgbToHwb(r: any, g: any, b: any) {
  var 
    h, 
    w, 
    bl,
...
function rgbToCmyk(r: any, g: any, b: any) {
  var 
    c, 
    m, 
    y, 
...
function w3trim(x: any) {
  return x.replace(/^\s+|\s+$/g, "");
}
function isHex(x: any) {
